...
def generate_tab_sprites(texture: Image.Image) -> dict[str, Image.Image]:
    ...
    The texture must be a square image with dimensions 16x16 or 32x32.
    ...
    elif texture.size[0] > 32:
        raise ValueError("Texture must be 32x32 or less")
    ...
    veritical_white_bar = Image.new("RGBA", (1, tab_h), WHITE)
    tab_selected_highlighted.paste(horizontal_white_bar, (1, 1))
    tab_selected_highlighted.paste(veritical_white_bar, (1, 1))
    tab_selected_highlighted.paste(veritical_white_bar, (tab_w - 2, 1))

    # Cut off 2x2 squares from the bottom corners of both images
    ...
    tab_highlighted.paste(veritical_white_bar, (1, 5))
    tab_highlighted.paste(veritical_white_bar, (tab_w - 2, 5))
...
def generate_separators(texture: Image.Image) -> tuple[Image.Image, Image.Image]:
    ...
    footer = texture.crop((0, 0, texture_l, 2))
    header = texture.crop((0, 2, texture_l, 4))
    ...
    footer_light = footer.crop((0, 0, texture_l, 1))
    footer_light = contrast(footer_light, 0.35)
    footer_light = brightness(footer_light, 0.75)

    header_light = header.crop((0, 0, texture_l, 1))
